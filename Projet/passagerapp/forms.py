...
class pform (UserCreationForm):
    class Meta:
        model = passager
        fields = ['cin','nom','prenom','num_tel','adresse','mail','date_naissance','username','photo' ,'role','password1','password2']
...
class uppform (forms.ModelForm):
    class Meta:
        model = passager
        fields = ['cin','nom','prenom','num_tel','adresse','mail','date_naissance','username','photo' ,'role']
    ...
    def __init__(self, *args, **kwargs):
        super(uppform, self).__init__(*args, **kwargs)
        for key, value in self.fields.items():
            value.widget.attrs['class'] = 'form-control'
